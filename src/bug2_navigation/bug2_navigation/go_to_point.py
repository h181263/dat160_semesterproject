...
class GoToPointClass(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(GoToPoint, 'go_to_point_switch', self.go_to_point_switch_callback)
    ...
    def go_to_point_switch_callback(self, request, response):
        self.active = request.move_switch
        self.desired_position = request.target_position
        response.success = True
        self.get_logger().info(f"GoToPoint active: {self.active}, Target position set to: {self.desired_position}")
        return response
